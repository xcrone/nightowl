...
/**
 * GET /api/apps/{app}/health — Agent Health (docs/pages/agent-health.md):
 * monitors NightOwl's own ingest pipeline, which has no telemetry table of
 * its own. Per the build decision ("real where a source exists; agent
 * health simulated") this synthesizes a stable, live-looking snapshot +
 * history for the selected period, keyed off the app id so each app reads
