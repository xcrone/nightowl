...
namespace App\Actions\Timeseries;
...
/**
 * GET /api/apps/{app}/timeseries/{metric} — bucketed time series for the
 * dashboard charts (docs/pages/app-dashboard.md). metric ∈ requests |
 * duration | exceptions | jobs.
 *
...
class ShowTimeseries
{
    ...
    public function handle(App $app, string $metric, ActionRequest $request)
    {
        ...
        $series = $rows->map(function ($r) use ($values) {
            $out = ['t' => Carbon::parse($r->bucket)->toIso8601String(), 'values' => []];
