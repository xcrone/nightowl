...
| `StoreApp` | Creates an `App` under a `Team`, minting a fresh unique opaque `app_id` and an `agent_token` (`App::generateAgentToken()`, the same `'nwt_'`-prefixed format `Settings\Actions\RegenerateAppToken` issues). Dispatches `App\Events\AppTokenIssued` (app_id + plaintext token) so `App\Listeners\SyncAppTokenToNightowl` can upsert a lookup row into `nightowl_apps` — the agent daemon reads it at boot to resolve its own `app_id` from its configured token (`agent/src/Support/AppIdResolver.php`). The event/listener live outside both `Apps` and `Settings` since `App` is shared by both (same reasoning as the model itself, below). Returns `AppResource` (same shape `ShowApp` embeds). | `authorize()`: membership on `$team->org`. `rules()`: `name` required, `description`/`environments` nullable. |
