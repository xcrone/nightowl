...
| `RegenerateAppToken` | Issues a fresh `nwt_`-prefixed agent token, returned in plaintext once. Dispatches `App\Events\AppTokenIssued`, the same event `Apps\Actions\StoreApp` fires — kept outside both domains' folders (`app/Events`, `app/Listeners`) for the same "`App` is shared by 2+ domains" reason the model itself isn't relocated (see Models section above), so neither domain reaches into the other's folder to dispatch/handle it. `App\Listeners\SyncAppTokenToNightowl` upserts a token-hash lookup row into `nightowl_apps` (on the `nightowl` connection) so the agent daemon can resolve its own `app_id` at boot (`agent/src/Support/AppIdResolver.php`). | `authorize()` always allows. No rules. |
